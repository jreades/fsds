...
- **Timed, Open Book Exam** ({{< var assess.quiz-weight >}} of module grade): A quiz delivered via Moodle requiring a mix of numeric and textual answers to short data analysis questions for which you must write the code.
...
- **Self-Evaluation** ({{< var assess.peer-weight >}} of module grade): A structured individual reflection in which students are asked to reflect on their *experience* of working as part of a data analytics team in order to identify behaviours and practices that helped/hindered the project, and then rate their peers on their contribution to the group's report.

::: {.notes}

**1** A PDF due {{< var assess.quiz-date >}} (after Reading Week). The assigned data set is the Inside Airbnb data for London.

**2** A two-part submission due {{< var assess.group-date >}}: 1) a Jupyter notebook containing all analysis and outputs (no length limit); 2) a PDF data-led policy briefing for the Mayor of London making use of the outputs from 1 . Sustainable authorship will be a factor in our marking.

**3** A PDF due {{< var assess.peerdate >}}. Sustainable authorship will be a factor in our marking.
