...
# > Spatial data connected in more complex ways unlocking even more applications, but at a cost!
...
```{python}
boros = gpd.read_file('https://bit.ly/3neINBV', driver='GPKG')
df    = pd.read_csv('https://bit.ly/3I0XDrq')
...
Sjoin adds an *predicate* (`'intersects','contains','within'`): 

```{python}
hackney = boros[boros.NAME=='Hackney']
rs = gpd.sjoin(gdf, hackney, predicate='within')
ax = hackney.plot(facecolor='none', edgecolor='blue', figsize=(8,4.5))
rs.plot(ax=ax, marker='*', color='red', markersize=5)
```
...
```{python}
rs = gpd.sjoin(gdf, hackney, how='left', predicate='within')
rs.NAME.fillna('None', inplace=True)
ax = boros[boros.NAME=='Hackney'].plot(edgecolor='k', facecolor='none', figsize=(8,4.5))
...
4. Adding conditions to joins makes them harder.
