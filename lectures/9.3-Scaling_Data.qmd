...
```{python}
ttc = pd.read_parquet('titanic.pq')
age = pd.cut(ttc['age'], [0, 18, 80])
pd.set_option('display.float_format', '{:,.3f}'.format)
ttc.pivot_table('survived', ['sex', age], 'class')
```

## In SQL

A little less immediately useful: 
...
sql = """PIVOT (
  ...
  ON class
  USING SUM(survived) AS survived
  GROUP BY sex, age"""
pd.set_option('display.float_format', '{:,.0f}'.format)
tmp = duck.sql(sql).to_df().set_index('sex')
tmp.columns = ['Age','First','Second','Third']
tmp
```
