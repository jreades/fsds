...
1. [Examining a data set](./audit.qmd) to determine its suitability for tackling a selected analytical 'problem' or 'challenge' (due {{< var assess.bio >}});
2. [Collaboratively writing a data-led policy briefing](./group.qmd) involving high-quality code, analysis, *and* presentation (both writing for an audience and data visualisation) suitable for use by a policy- or decision-maker ({{< var assess.group >}});
3. [Reflecting on the process](./individual.qmd) to better-understand *why* a project succeeded/failed so as to improve future outcomes (due {{< var assess.individual >}}.
