...
This should (I hope) be trivial to read now and (I hope) you have what seems like pretty good performance in the range of 150-175 milliseconds.
...
Whoa! Notice the difference in the time needed to complete the operation!!! We just cut the time by upwards of 80%!
...
The timing code should *already* show a [significant improvement over pandas-only code](#a-simple-query) (on my laptop it is 90ms vs. 160ms), and now let's unpack what's happening:
...
Even though it looked like pandas performed *massively* better when we updated our query to [filter then load](#filter-then-load), DuckDB is **even faster**! I get that DuckDB takes about 25% the time that pandas does.
