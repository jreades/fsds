...
```{python}
# your code here
df = pd.read_parquet( cache_data(url, Path('data/clean').resolve()) )
print(f"Data frame is {df.shape[0]:,} x {df.shape[1]}")
```
...
#| echo: false
#| output: asis
print(f"So, starting with <code>fn = '{ymd}-{city}-listings.gpkg'</code>")
```

```{python}
#| echo: false
fn = f"{ymd}-{city}-listings.gpkg"
```
...
gpkg_out = Path(f'data/geo/{fn}')
print(f"Saving geopackage data to {gpkg_out.resolve()}...")
try:
...
You *may* find that saving to a GeoPackage fails with a `TypeError` because of the presence of Categorical data. THis behaviour seems to have changed recently so that it may no longer be an issue, but if you do see an error this is why.

But the below, in which we specify as a 'geoparquet' because of the coordinate data, does not:

```{python}
pq_out = Path(f'data/geo/{fn.replace('.gpkg','.geoparquet')}')
print(f"Saving geopackage data to {pq_out.resolve()}...")
gdf.to_parquet(pq_out)
print("Saved.")
```
...
```{python}
ax = gdf[ (gdf.lnprice > 0) & (gdf.lnprice < np.log(1000)) ].plot(figsize=(8,6), marker='*', markersize=0.25,
