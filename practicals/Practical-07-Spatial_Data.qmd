...
We're building on the work done in [Practical 5](Practical-06-Numeric_Data.ipynb) and [Practical 4](Practical-05-Objects.ipynb) (with a particular nod to the [lecture on Data](https://jreades.github.io/fsds/sessions/week5.html#lectures)) to create some useful functions that we can call on at-need to improve the ease of doing data analysis.
...
```{python}
# Oooooh, let's use a *pandas* method here
gdf[gdf.price < 1000].price.plot.hist(bins=200, figsize=(8,3)); 
```
...
```{python}
ax = gdf.plot(figsize=(8,6), marker='*', markersize=0.25,
