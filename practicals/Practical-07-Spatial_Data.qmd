...
We're building on the work done in previous practicals with a particular nod to the [lecture on Data]({{< var module.web >}}sessions/week5.html#lectures) to create some useful functions that we can call on at-need to improve the ease of doing data analysis.

:::

I find GeoPackages and GeoParquet to be by far the easiest way to distribute geo-data now: they both use a single file in a database-like format and include the projection information by default. In some cases QGIS can even embed information about rendering style! 

We're going to do something _similar_ to the `get_url` in order to download the file to our hard drive and save it there. The improvement is that we'll check to see if the file already exists and, if it does, return that so that you can don't have to keep downloading the same data week after week. 
...
water = gpd.read_file( cache_data(spath + 'Water.gpkg?raw=true', ddir.resolve()) )
green = gpd.read_file( cache_data(spath + 'Greenspace.gpkg?raw=true', ddir.resolve()) )
...
```{python}
# your code here
df = pd.read_parquet( cache_data(url, Path('data/raw')) )
print(f"Data frame is {df.shape[0]:,} x {df.shape[1]}")
```
...
Right, we're finally there! We need to convert our coordinates into some kind of geo-data. GeoPandas offers two ways to do this: the original way using `zip` and a new utility method called `points_from_xy`. Here's the old way **which still works but shouldn't be used**:
...
The old way is interesting because it's very explicit about what you're doing (creating a list of Points that will be come a new GeoSeries), but it does not allow you to set a projection, unlike the new approach with the 'helper function' from the module **which is what you should use**:
...
| GeoParquet | Highly compressed columnar database | Limited GIS support (except QGIS), and DuckDB has issues.^[You can install the [Spatial Extension](https://duckdb.org/docs/stable/core_extensions/spatial/overview.html). |
...
Since there are _many_ formats in which to save geo-data, rather than have multiple `to_format_x` methods, GeoPandas has tried to have _one_ for local files (`to_file`). Normally, Geopandas will do the 'right thing' based on the extension: so if it sees `.shp` it will use the ESRI Shapefile driver, if it sees `.gpkg` it will use the Geopackage dirver, and the same for `.geojson`, etc.

There are two ways you *may* run into trouble:

1. If you are reading/writing a **URL** (e.g. [https://github.com/jreades/.../src/Boroughs.gpkg?raw=true](https://github.com/jreades/fsds/blob/master/data/src/Boroughs.gpkg?raw=true)) where there is content *after* the `.gpkg` exten sion. If in doubt, specify the driver.
2. If you are reading/writing a **Parquet** file then you need to use `to_parquet()` and `read_parquet()` for compatibility with Pandas.
...
# Print out first 399 characters of the string
...
print(boros.geometry.centroid.iloc[1]) 

# And here's a point
boros.geometry.centroid.iloc[1]
```
...
Print out the projection of each GeoDataFrame (the water, borough, greenspace, and InsideAirbnb data) using a for loop.
...
```{python}
for g in [boros,water,green,gdf]:
    print(g.crs)
```

::::

You should see that three are in the [EPSG:27700 CRS](https://epsg.io/27700) which is a common one for analysis using GB data, but the InsideAirbnb data is not.
...
Let's view the CRS information for the InsideAirbnb data in its original projection...
...
... and reproject this into the `OSGB1936/BNG` CRS:
...
bounds = ', '.join([f"{x:.5f}" for x in gdf.to_crs('epsg:27700').total_bounds])
print(f"The bounding box is: {bounds}")
```
...
Behind the scenes, GeoPandas is using `matplotlib` to render the map, so let's play with the colours to get the _start_ of something map-like. You will want to look both at [how to make maps in GeoPandas](https://geopandas.org/mapping.html) and  at the different ways to [specify colours in Matplotlib](https://matplotlib.org/stable/users/explain/colors/colors.html). For the greenspace map you are looking for information about tuples... which can have three or four elements.

## Making Plots

We'll see how you control figure-making more effectively later, but for now let's just see what they look like using GeoPanda's plotting functionality. You should output three plots from the loop.

:::: {.qna}

#### Question

Can you plot the 'boros', water, and green space in a loop?
...
```{python}
gdf.to_crs('epsg:27700').plot(column='price', cmap='viridis', alpha=0.25, markersize=1, figsize=(8,6));
```
...
In the second case we're filtering the data *before* sending to be plotted, so there is not data above $1,000/night and the 200 bins applies to data between $0 and $1,000. In the first case, we only apply the limits to the plot, not the data, so it's truncating the graph *after* binning rather than dividing the range between 0 and 1,000 into 200 bins.
...
You can do the same thing with a `box`plot:
...
```{python}
ax = gdf.price.plot.box(vert=False, figsize=(8,3))
ax.set_xlim(0,2500)
```
...
3. Work out how to fix the error and *then* repeat step 1.
...
```{python}
ax = gdf[ 
    (gdf.lnprice > 0) & (gdf.lnprice < np.log(1000)) 
].plot(figsize=(8,6), marker='*', markersize=0.25,
...
# pass ax=ax to gdf.plot -- this has to do with
# where and how things are plotted.
gdf[ (gdf.lnprice > 0) & (gdf.lnprice < np.log(1000)) ]
f,ax = plt.subplots(1,1,figsize=(8,6))
gdf.plot(ax=ax, marker='*', markersize=0.25,
         column='lnprice', cmap='viridis',
         legend=True, legend_kwds={'label':'Natural Log of Price per Night ($)'});
...
With luck, that will give you a dynamic, scrollable map!
...
To give a bit of a show of how we can put it all together try to get this working for something approximating a half-decent map.


Note that the following command seems to produce a lot of warnings for me: reading closely, I think it's about changes to Python with 3.12 and higher which have triggered warnings to do with syntax that _was_ perfectly legitimate with Python 3.10 but is now more strongly deprecated.
...
# Reload the data because we've filtered it by price, not geography
gdf = gpd.read_parquet(Path(f'data/geo/{ymd}-{city}-listings.geoparquet').resolve()).to_crs('epsg:27700')
...
# (k=4) using the original price. You could use
...
a = ax.text(tgdf.geometry.x.max(), tgdf.geometry.y.min(), 'Source: InsideAirbnb (2025)', 
