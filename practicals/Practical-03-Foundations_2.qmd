...
While there's no benefit to signing up early, if you've already formed a group then you can now [Sign Up](https://forms.office.com/e/2ij0sWHnpR)!
...
**Stop!**: There are two problems to the `data.splitlines()` and `row.split(',')` approach! One of them is visible (though not obvious) in the examples above, the other is not.
