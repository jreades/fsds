...
Now this isn't the *best* function because we might expect it to return integers for all number-like things regardless of whether they are floats or integers or strings, but at least we've not thrown an *exception* when this happens, we've only converted the fields that clearly apply! We'll copy the data to new data structure only so that we know we're not overwriting `ds` until we're sure that the code works.
...
At this point it's worth asking: was all this *worth* it? Well let's see! The best way to test this is to use a *different* data set and see if we've solved the 'hard-coding' problem.
