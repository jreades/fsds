...
# at the same location (i.e. {{< meta data.year >}}-{{< var data.month >}}-{{< var data.day >}}-calendar.csv.gz)
host = 'https://orca.casa.ucl.ac.uk'
path = '~jreades/data/{{< var data.fn >}}'
url  = f'{host}/{path}'
...
- `df.memory_usage(index=True, deep=False)` -- memory usage details about each column (can be quite slow as it's doing a *lot* of digging)
...
To recap, when we use the `[[...]]` syntax we're taking a short-cut through the data by column (keeping all rows). The *full* syntax is `df[<row_selection>,<col_selection>]`. When we *don't* specify both does then it defaults to `df[<col_selection>]`. 
