...
#| echo: False
import pandas as pd
df = pd.read_csv('https://orca.casa.ucl.ac.uk/~jreades/data/Listings-lg.csv')
df.head()
```
...
::::
To fFind the `mean` number of bedrooms we can use a package called numpy. `numpy` (Numerical Python) is used _so_ much that most people simply refer to it as `np`. This is a _huge_ package in terms of features, but right now we're interested only in the simple arithmatic `mean`.
