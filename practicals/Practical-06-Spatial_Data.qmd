...
So, starting with `fn=`{python} f"'{fn}'"``...
...
```{python}
# Oooooh, let's use a *pandas* method here
gdf.price.plot.hist(bins=200, figsize=(10,3)); 
```
...
```python
ax = gdf.price.plot.??(bins=??, figsize=(10,3));
ax.??
```
...
London Airbnb Listings Price Per Night (Quantiles)
